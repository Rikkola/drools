...
public abstract class BaseFEELFunction
    ...
    public Object invokeReflectively(final EvaluationContext ctx,
        ...
        // TODO add errors for invalid parameter count, invalid parameter type, named parameter present that is not on the list, invalid parameter names and invalid method name
    ...
    private FEELFnResult<String> invokeStringFunction(final Object[] params) {
        if (isNamedParams(params)) {
            if (params.length == 1) {
                ...
                // TODO test if p needs to be set
