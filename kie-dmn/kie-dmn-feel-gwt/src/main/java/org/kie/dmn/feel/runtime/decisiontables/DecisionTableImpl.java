...
public class DecisionTableImpl implements DecisionTable {
    ...
    public static Map<Integer, String> checkResults(List<? extends OutputClause> outputs, EvaluationContext ctx, List<? extends Indexed> matches, List<Object> results) {
        ...
        for( Object result : results ) {
            if( outputs.size() == 1 ) {
                ...
                int outputIndex = 1;
                for ( OutputClause output : outputs ) {
     ...
     */
    private static void checkOneResult(EvaluationContext ctx, Indexed rule, Map<Integer, String> msgs, OutputClause dtOutputClause, Object result, int index) {
    ...
    private static void checkOneValue(EvaluationContext ctx, Indexed rule, Map<Integer, String> msgs, OutputClause dtOutputClause, Object value, int index) {
        // TODO EvaluationContextImpl is not visible here
...
//        }
        ...
        for( int i = 0; i < params.length; i++ ) {
            ...
            if ( input.getInputValues() != null && ! input.getInputValues().isEmpty() ) {
                ...
                if ( !satisfies ) {
                    ...
                    return Either.ofLeft(new InvalidInputEvent( Severity.ERROR,
    ...
    private List<DTDecisionRule> findMatches(EvaluationContext ctx, Object[] params) {
        ...
        ctx.notifyEvt( () -> {
            ...
            return new DecisionTableRulesMatchedEvent(Severity.INFO,
    ...
    private boolean matches(EvaluationContext ctx, Object[] params, DTDecisionRule rule) {
        for( int i = 0; i < params.length; i++ ) {
            CompiledExpression compiledInput = inputs.get(i).getCompiledInput();
            // TODO I Really have the feeling CompiledFEELExpression should be visible here
