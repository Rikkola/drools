...
public class DroolsVerifierDTValidator {
    ...
    private static AnalyzerConfiguration analyzerConfiguration = new DMNAnalyzerConfiguration(CheckConfiguration.newDefault());
    ...
    public Set<Issue> validateDT(DMNModel model, DecisionNode dn, DecisionTable dt) {
        final IndexImpl index1 = new IndexImpl();
        final ObjectType objectType = new ObjectType(OBJECT_TYPE_NAME, analyzerConfiguration);
        int jHeadIdx = 0;
        final List<Column> columns = new ArrayList<>();
        final List<ObjectField> objectFields = new ArrayList<>();
        for (InputClause ic : dt.getInput()) {
            final Column c = new Column(jHeadIdx + 1, analyzerConfiguration);
            columns.add(c);
            final String typeRef = Optional.ofNullable(ic.getInputExpression().getTypeRef()).map(QName::getLocalPart).orElse("any");
            final ObjectField of = new ObjectField(OBJECT_TYPE_NAME, typeRef, typeRef, analyzerConfiguration);
            objectFields.add(of);
        ...
        for (OutputClause oc : dt.getOutput()) {
            final Column c = new Column(jHeadIdx + 1, analyzerConfiguration);
            columns.add(c);
            final String typeRef = Optional.ofNullable(oc.getTypeRef()).map(QName::getLocalPart).orElse("any");
            final ObjectField of = new ObjectField(OBJECT_TYPE_NAME, typeRef, typeRef, analyzerConfiguration);
            objectFields.add(of);
            objectType.getFields().add(of);
            jHeadIdx++;
        ...
        for (int jRowIdx = 0; jRowIdx < dt.getRule().size(); jRowIdx++) {
            ...
            for (UnaryTests ie : r.getInputEntry()) {
                final ObjectField objectField = objectFields.get(jColIdx);
                ...
                for (BaseNode n : utln.getElements()) {
                    ...
                    final UnaryTestNode ut = (UnaryTestNode) n;
                    if (ut.getOperator() == UnaryOperator.EQ || ut.getOperator() == UnaryOperator.GT || ut.getOperator() == UnaryOperator.GTE || ut.getOperator() == UnaryOperator.LT || ut
                        ...
                        final FieldCondition<?> condition1 = new FieldCondition<>(field,
                                                                                  columns.get(jColIdx),
                        ...
                        field.getConditions().add(condition1);
                        row.getConditions().add(condition1);
                        final FieldCondition<?> condition2 = new FieldCondition<>(field,
                                                                                  columns.get(jColIdx),
            ...
            for (final LiteralExpression oe : r.getOutputEntry()) {
                final ObjectField objectField = objectFields.get(jColIdx);
                final Field field = new Field(objectField, objectField.getFactType(), objectField.getFieldType(), objectField.getName(), analyzerConfiguration);
                fields.add(field);
                pattern.getFields().add(field);

                final ProcessedExpression compileUnaryTests = (ProcessedExpression) FEEL.compile(oe.getText(), FEEL.newCompilerContext());
                final InterpretedExecutableExpression interpreted = compileUnaryTests.getInterpreted();
                final BaseNode baseNode = (BaseNode) interpreted.getASTNode();
        ...
        final Set<Issue> result = new HashSet<>();
