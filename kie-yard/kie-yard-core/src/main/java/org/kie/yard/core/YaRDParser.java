...
public class YaRDParser {
    ...
    public YaRDParser(String yaml) throws Exception {
        this.yaml = yaml;
        model = getModel(yaml);
        parse(yaml);
    }
    ...
    private YaRD getModel(String yaml) throws Exception {
        return new YaRD_YamlMapperImpl().read(yaml);
    }

    private YaRDDefinitions parse(String yaml) throws Exception {
