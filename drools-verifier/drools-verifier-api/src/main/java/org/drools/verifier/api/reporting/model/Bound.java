...
public class Bound<V extends Comparable<V>> implements Comparable<Bound<V>> {
    ...
    public int compareTo(Bound<V> o) {
        ...
        if (this.boundaryType == o.boundaryType) {
            return 0;
        } else if (this.boundaryType == Range.RangeBoundary.OPEN) {
            return -1;
        } else {
            return 1;
        }
    ...
    public String toString() {
        if (isLowerBound()) {
            return (boundaryType == Range.RangeBoundary.OPEN ? "(" : "[") + value;
        } else {
            return value + (boundaryType == Range.RangeBoundary.OPEN ? ")" : "]");
        }
    }
